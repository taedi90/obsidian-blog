...
## 2. 이슈
Docker Compose 의 Healthcheck 옵션을 사용하면 상태 이상에 빠진 컨테이너를 확인할 수는 있지만, 단순히 `unhealthy` 로 표시만 될 뿐 컨테이너가 자동으로 재기동 되지 않는다. 컨테이너 재기동을 위해서는 Kubernetes 나 swarm mode 와 같이 오케스트레이션 툴을 사용하는 것이 가장 깔끔한 방법이겠지만, 언제나 그렇듯 인프라 구성은 항상 내 뜻대로 할 수 없다.  
때문에 Docker 환경에서 컨테이너가 Healthcheck 실패 시 Auto healing 을 부분적으로라도 흉내 낼 수 있는 방법을 알아봤다.  

## 3. 해결
방법은 크게 두 가지로 생각해볼 수 있었다.
1. 컨테이너 외부(호스트)에서 unhealthy 상태의 컨테이너를 cron 등으로 모니터하고 재기동 하는 방법
