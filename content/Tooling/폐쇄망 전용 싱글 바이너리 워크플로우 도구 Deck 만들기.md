...
솔직히 절반은 시행착오였다. 처음엔 이것저것 다 되는 범용 도구를 지향했다가, "그건 Ansible이 이미 더 잘한다"는 벽에 계속 부딪혀 기능을 <b>덜어내는</b> 방향으로 되돌아왔다. 미출시 프로젝트라 하위 호환을 신경 쓰지 않은 게 이때 크게 도움이 됐다. 어제 만든 걸 오늘 미련 없이 지울 수 있었다.
