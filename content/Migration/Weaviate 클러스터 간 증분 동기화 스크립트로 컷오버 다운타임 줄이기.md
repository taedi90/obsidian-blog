...
Weaviate 클러스터를 통째로 다른 클러스터로 옮기는 일을 맡았다. 노드 이름이 바뀌는 사례였기 때문에 파일 복사(재부착)는 불가능했고, backup/restore로 논리 이관을 수행해야 했다. 여기서 다운타임 문제가 걸렸다. 이 스크립트는 그 다운타임을 줄이려고 만든 보조 도구다. 이관 전체 절차를 다루지 않고, "증분을 어떻게 따라잡았나"만 떼어 적는다.

## 1. backup/restore의 한계

Weaviate(OSS)에는 클러스터 간 실시간 복제 기능이 없다. 공식 이관 수단은 backup/restore 하나뿐인데, backup은 늘 전체 스냅샷이다. "지난번 이후 바뀐 것만" 같은 증분 백업은 존재하지 않는다.

그러면 컷오버(구 클러스터 → 신 클러스터 전환) 시나리오는 이렇게 진행된다. 쓰기를 멈추고, 전체 백업을 생성하고, 신 클러스터에 복원하고, 엔드포인트를 넘긴다. 문제는 이 <b>쓰기 정지 시간이 데이터 크기에 비례</b>한다는 점이다. stg에서 재본 결과 608 클래스·19GB 기준으로 백업에 수십 분, restore 마무리에 수 분이 걸렸다. 데이터가 더 크면 그만큼 소요 시간도 늘어난다. 서비스를 그 시간만큼 세워둘 수는 없었다.

방법은 흔한 패턴이다. 미리 backup/restore로 새 클러스터를 채워두고(시딩), 그 이후 생긴 변경분만 따로 따라잡은 뒤, 컷오버 때는 남은 소량만 반영한다. 그러면 중단창이 "전체 복원 시간"에서 "마지막 델타 반영 시간"으로 줄어든다. 문제는 그 "변경분만 따라잡는" 도구가 Weaviate에는 없다는 것이다. 그래서 직접 만들었다.

## 2. Weaviate가 안 주는 것 두 가지

증분 동기화를 작성하려면 보통 두 가지 정보가 필요하다. "무엇이 바뀌었는지"와 "무엇이 지워졌는지"다. Weaviate는 이 둘을 순순히 제공하지 않는다.
...
한 가지 전제가 있다. 대상 클래스가 `vectorizer=none`(벡터를 직접 주입하는 방식)이어야 한다. 저장된 벡터를 그대로 다시 넣기 때문에 재벡터화가 일어나지 않는다. 임베딩을 서버가 만드는 구성이면 이 전제가 깨진다.
 ...
 > [!INFO]
 > 이관 얘기를 하다 보면 Collection aliases나 async replication을 떠올리는 사람이 있는데, 둘 다 이 문제에는 맞지 않았다. aliases(1.32+)는 한 클러스터 "안"에서 컬렉션 이름을 바꿔 끼우는 기능이고, async replication(1.29 GA)은 한 클러스터 "안" 복제본 노드 간 일관성용이다. 어느 쪽도 클러스터-대-클러스터 이관 도구가 아니다.
...
이 과정은 소스·타깃을 양쪽 다 전량 순회하므로 느리다. 그래서 평소 델타 반영에는 끄고, append-only가 아닌 이상 컷오버 정합 단계에서만 켰다. 앱팀이 삭제 빈도를 확답하지 못하면 켜두는 쪽을 택했다. "지운 문서가 검색에 계속 나오는" 조용한 버그가 발생하는 손해보다 이 옵션을 켜 두는 것이 낫다고 판단했다. 안전장치로 구 클러스터와 Retain PV는 검증이 끝날 때까지 남겨 두었다.

## 6. dry-run과 컷오버에서 쓰는 법

실제 쓰기 전에 항상 `--dry-run`으로 건수부터 확인했다. 반영 없이 "이번에 몇 건이 델타인지, 몇 건이 삭제 대상인지"만 세어 출력한다. 컷오버 창을 잡을 때 마지막 델타가 얼마나 남았는지 가늠하는 근거가 된다.
...
실제 흐름은 이렇게 진행됐다. backup/restore로 시딩한 뒤 시딩 시각을 `--since`로 넣어서 델타를 여러 번 실행해 남은 변경분을 줄였고, 컷오버 때 쓰기를 잠깐 멈춘 다음 마지막 델타 + 삭제 대조를 한 번 더 실행했으며, 앱이 바라보는 엔드포인트를 신 클러스터로 넘겼다.

API 키는 `--api-key` 인자나 `WEAVIATE_API_KEY` 환경변수로만 받고, <b>어떤 로그에도 값을 출력하지 않는다</b>. 소스·타깃 키가 다르면 `--source-key`/`--target-key`로 따로 지정한다. 의존성은 없다. `urllib` 등 표준 라이브러리만 사용해서, 폐쇄망에 파이썬 인터프리터만 있으면 그대로 동작한다. pip으로 무엇을 가져올지 협의하는 수고를 덜려고 일부러 그렇게 작성했다.

한계는 분명하다. 이것은 실시간 복제가 아니라 "실행할 때마다 그 시점까지 따라잡는" 폴링형 도구이고, 서버 시간 필터가 불안정해서 매번 전량을 순회한다. 삭제 정합은 더 큰 비용이 든다. 그래도 backup/restore 하나로 다운타임을 데이터 크기에 통째로 묶어 두는 것보다는 훨씬 나았다.
