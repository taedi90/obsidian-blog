...
"본사 `app-01` 서버가 느려지고 SSH도 잘 연결되지 않는다"는 제보를 받았다. 처음에는 서버 부하 문제인 줄 알았는데, 로그인 실패 로그부터 확인하니 상황이 달라졌다.
...
특정 해외 대역에서 SSH 계정을 바꿔 가며 무차별 접속 시도를 하고 있었다. 서버가 느려진 이유는 부하 문제가 아니라, 이 무차별 대입(brute force) 시도가 SSH 데몬과 네트워크 자원을 계속 소모하고 있었기 때문이었다.

문제는 이 SSH가 공인 IP의 비표준 포트(`39022`)로 열려 있었다는 점이다. 포트를 변경해두었다고 회피되는 것이 아니라, 스캐너는 열린 포트를 전수로 조사한다. 결국 방화벽 단에서 공격 대역 자체를 차단해야 했다.

## 3. 해결

### 1. 공격 트래픽 특정

먼저 트래픽이 어디서 오는지부터 정확히 확인했다. Fortigate GUI에서는 <b>FortiView → Sources</b>의 `Traffic From WAN` 탭으로 외부에서 들어오는 소스 IP를 정렬해봤고, CLI에서는 sniffer로 해당 포트만 필터링했다.
...
공격 소스가 특정 해외 국가 2곳과, 그중 유독 집요하게 공격하던 `198.51.100.0/24` 서브넷으로 좁혀졌다. 이 대역을 인바운드에서 차단하기로 했다.

### 2. 인바운드 차단 규칙 추가

Fortigate에서 국가(Geo) 오브젝트와 서브넷 주소 오브젝트를 만들어, 이들을 source로 하는 <b>deny 정책</b>을 WAN 인바운드에 추가했다. 여기까지는 평범한 절차다. 그런데 추가한 뒤에도 `btmp`에 실패 로그가 계속 쌓였다. 차단이 적용되지 않은 것이다. 여기서 두 가지 함정에 부딪혔다.

### 3. 함정 하나: 정책 평가 순서

Fortigate는 방화벽 정책을 <b>위에서 아래로 순차 평가</b>하다가 처음 매칭되는 규칙에서 멈춘다. 정책 ID 숫자가 작다고 먼저 적용되는 게 아니다. GUI 목록에 보이는 <b>순서</b>가 곧 평가 순서다.

내가 만든 deny 정책이 기존 허용(allow) 정책보다 <b>아래</b>에 있었다. 그러므로 공격 트래픽이 위쪽 허용 정책에 먼저 매칭되어 통과하고, 아래의 deny까지 도달하지 못했다. deny 규칙을 해당 허용 규칙보다 위로 옮기자 비로소 평가 순서상 먼저 적용되었다.

> [!IMPORTANT]
> 차단 규칙은 그것을 무력화하는 허용 규칙보다 반드시 <b>순서상 위</b>에 두어야 한다. 기준은 정책 ID(번호)가 아니라 목록에서의 위치다. 이 조건 하나로 한참 헤맸다.

### 4. 함정 둘: VIP(DNAT) 트래픽의 destination

순서를 수정했는데도 SSH 대역은 여전히 통과되었다. 이유는 이 SSH가 <b>Virtual IP(VIP)</b>를 통한 포트포워딩, 즉 Destination NAT였기 때문이다.
...
내가 처음 만든 deny 정책은 destination이 `all`이었다. VIP를 거쳐 들어오는 SSH 트래픽은 `all`에 매칭되지 않아 이 규칙을 그냥 지나쳤고, 뒤이어 VIP를 destination으로 가진 허용 정책에 매칭되어 통과했다. deny 정책의 destination을 `all`에서 해당 <b>VIP 정책(오브젝트)</b>으로 변경하고, 순서도 허용 정책 위에 두자 비로소 공격 트래픽이 실제로 drop되기 시작했다.
...
sniffer로도 재차 확인했다. 차단한 서브넷에서 `39022`로 들어오던 패킷이 사라졌고, 서버 응답 속도도 원래대로 돌아왔다.
...
정책 자체는 처음부터 존재했다. 순서와 destination 두 항목을 동시에 맞추기 전까지는 "규칙은 추가했는데 왜 차단되지 않는가"만 반복했을 뿐이다.
