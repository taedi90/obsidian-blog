...
증상은 봤으니 원인을 찾아야 하는데, "NFS가 느리다"는 심증만으로는 손을 못 댄다. 디스크가 찬 건지, 파일 디스크립터가 샌 건지, 커널 레벨에서 무엇이 막힌 건지, 아니면 진짜 NFS 워크로드 자체가 과한 건지를 확인해야 하므로, 층을 하나씩 걷어내며 확인하기로 했다.
...
여기서 원인이 드러났다. CPU wait(`wa`)가 11~13%로 붙어 있고, `vmstat`의 `b`(uninterruptible sleep, 대개 I/O 대기) 컬럼에 프로세스가 잡혔다. CPU가 유휴 상태로 놀고 있는 것이 아니라 디스크 응답을 기다리며 묶여 있다는 뜻이다. 로드 애버리지가 높았던 것도 이 대기 프로세스들이 큐에 쌓여서였다.
...
여러 개의 DB 볼륨이 동시에 NFS로 쓰기 요청을 보내고 있었다. 여기서 원인이 분명해졌다. DB는 랜덤 I/O와 잦은 fsync를 요구하는 대표적인 쓰기 집약 워크로드인데, NFS는 본래 네트워크 파일 공유를 위한 저장 방식이다. 대용량 파일을 순차로 읽고 쓰거나 여러 클라이언트가 문서를 공유하는 용도에는 적합하지만, DB 데이터 디렉토리를 얹기에는 적합하지 않았다. 그런 디렉토리를 여러 개 얹었으니 네트워크 스토리지가 감당하지 못한 것이다.
...
`dirty_ratio`를 낮춘 이유는, 기본값(보통 20% 안팎)에서는 더티 페이지가 크게 쌓였다가 한꺼번에 디스크로 흘러들면서 그 순간 I/O 부하가 급격히 증가하기 때문이다. 임계치를 낮추면 쓰기를 조금씩 자주 처리하는 대신 순간적인 폭주를 줄입니다. 이는 근본 해결이 아니라 순간 부하를 잘게 나누는 수준의 완화책이다.

효과는 있었다. 로드가 20대에서 한 자릿수로 내려왔다. 다만 이건 시간을 버는 조치지 답이 아니라는 걸 알고 있었다. DB 쓰기가 네트워크 너머로 나가는 구조 자체는 그대로였으니까.

### 3. 스토리지 용도 분리

결국 수정해야 할 대상은 스토리지클래스를 하나로 통일해둔 설계였다. 편의를 위해 만든 단일 구성이 병목의 원인이었다. 그래서 용도에 따라 스토리지를 나눴다.

- <b>DB 데이터</b>: 랜덤 I/O와 fsync가 잦으므로 네트워크 스토리지에 두면 안 된다. 노드 로컬 디스크(SSD)를 사용하는 스토리지클래스로 옮긴다. 로컬 경로를 그대로 사용하는 `local-path-provisioner`를 쓰거나, 복제와 스냅샷이 필요하면 Longhorn 같은 분산 블록 스토리지를 사용한다.
...
DB StatefulSet의 볼륨클레임 스토리지클래스를 로컬 기반으로 바꾸고 데이터를 옮기는 것이 근본 대책이다. 로컬 디스크 때문에 파드가 특정 노드에 묶이는 스케줄링 제약이 생기지만, DB처럼 상태를 가진 워크로드는 어차피 임의의 노드를 자유롭게 오가도록 두는 것이 바람직하지 않으므로 큰 제약은 아니다.

우선 NFS 노드에 taint를 설정하여 DB나 다른 파드가 다시 스케줄링되지 못하게 막고, 공유와 백업 용도로만 남겼다. 네트워크 버퍼와 NFS 스레드 수를 연결량에 맞춰 조정해두긴 했지만, 근본적인 해결책은 어디까지나 "DB를 NFS에서 빼는 것"이다. 튜닝만으로 해결될 문제였다면 애초에 여기까지 오지 않았을 것이다.
...
편의를 위해 스토리지를 하나로 통일해둔 것이 문제의 원인이었다. 워크로드마다 알맞은 스토리지가 따로 있다는 사실을, DB 데이터를 NFS에 맡긴 결과를 직접 겪고 나서야 확인했다.
