...
- **OS**: Rocky Linux 9
- **Platform**: Kubernetes Worker Node
- **Hardware**: Intel e1000e 네트워크 인터페이스 카드(NIC)

## 💬 이슈

**Rocky Linux 9** 기반의 **Kubernetes Worker Node**에서 Intel **e1000e** NIC 드라이버를 사용하던 중 문제가 발생했다. 이 드라이버는 특정 상황에서 하드웨어 정지(hang)를 유발하는 고질적인 이슈가 있는 것으로 판단했다.

특정 서버 노드의 `dmesg`나 서버 콘솔에서 `e1000e: detected hardware unit hang` 오류 메시지가 지속적으로 출력되는 현상을 마주했다. 문제가 발생하면 서버의 네트워크 통신이 완전히 마비되어 Kubernetes 클러스터에서 노드가 `NotReady` 상태로 전환되었고, SSH 접속을 포함한 모든 원격 접근이 불가능해지는 심각한 상황으로 이어졌다.

- **주요 증상**:
    - 서버의 네트워크 인터페이스가 응답하지 않았다.
    - Kubernetes 클러스터에서 해당 노드가 **`NotReady`** 상태로 변경되었다.
    - **SSH**를 포함한 모든 네트워크 연결이 불가능해져 물리 콘솔 또는 **IPMI**를 통해서만 접근해야만 했다.

- **특이사항**:
    - 동일한 하드웨어와 커널 버전을 사용하는 다른 노드에서는 문제가 발생하지 않아, 특정 노드에서만 문제가 재현되었다. 이는 트래픽 부하, 케이블링, 또는 특정 하드웨어의 미세한 차이 등 복합적인 요인이 원인일 수 있다고 보았다.

## 🧗 해결

이 문제를 해결하기 위해 여러 조치를 시도했다. 각 단계는 독립적으로 또는 복합적으로 적용될 수 있으며, 나의 경우 **2번과 3번 방법을 함께 적용**하여 최종적으로 문제를 해결할 수 있었다.

### 1. 네트워크 트래픽 분산 (임시 조치)

가장 먼저 시도한 것은 문제의 원인이 과도한 트래픽일 가능성을 줄여보는 것이었다. 이를 위해 로드 밸런서(FortiGate Virtual Server)의 분배 방식을 조정했다.

- **방법**: 기존의 균등 분배(Round Robin) 방식 대신, 문제가 발생한 노드의 가중치(weight)를 낮추고 안정적인 다른 노드의 가중치를 높여 트래픽을 최소화했다.
- **결과**: 이 방법은 일시적으로 증상을 완화할 수는 있었지만, 근본적인 해결책은 아니라고 판단했다.

### 2. 커널 파라미터 조정 (전력 관리 기능 비활성화)

조사 결과, e1000e 드라이버의 절전 기능이 하드웨어 정지(hang)의 주요 원인으로 지목되는 경우가 많았다. 따라서 관련 전력 관리 기능을 비활성화하여 안정성을 확보하고자 했다.

> [!IMPORTANT]
> **e1000e hang 이슈의 가장 유력한 원인**은 드라이버의 전력 관리 기능과 관련된 것으로 알려져 있다. 대부분의 경우 이 기능을 비활성화하는 것만으로도 문제가 해결될 수 있다.
...
**TCP 분할 오프로드(TCP Segmentation Offload, TSO)**, **일반 분할 오프로드(Generic Segmentation Offload, GSO)** 등과 같은 네트워크 오프로딩 기능이 드라이버와 충돌을 일으킬 수 있다고 보았다. 이 기능들은 CPU의 부하를 줄여주지만, 특정 드라이버와 호환성 문제를 일으키기도 한다.
...
모든 조치를 적용한 후, 설정이 올바르게 반영되었는지 확인하는 과정은 필수적이다.
