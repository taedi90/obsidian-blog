...
- <b>Platform</b>: Kubernetes Worker Node
- <b>Hardware</b>: Intel e1000e 네트워크 인터페이스 카드(NIC)

## 💬 이슈

<b>Rocky Linux 9</b> 기반의 <b>Kubernetes Worker Node</b>에서 Intel <b>e1000e</b> NIC 드라이버를 사용하던 중 문제가 발생했다. 이 드라이버는 특정 상황에서 하드웨어 정지(hang)를 유발하는 고질적인 이슈가 있는 것으로 판단했다.
...
- <b>주요 증상</b>:
    ...
    - Kubernetes 클러스터에서 해당 노드가 <b>`NotReady`</b> 상태로 변경되었다.
    - <b>SSH</b>를 포함한 모든 네트워크 연결이 불가능해져 물리 콘솔 또는 <b>IPMI</b>를 통해서만 접근해야만 했다.
...
이 문제를 해결하기 위해 여러 조치를 시도했다. 각 단계는 독립적으로 또는 복합적으로 적용될 수 있으며, 나의 경우 <b>2번과 3번 방법을 함께 적용</b>하여 최종적으로 문제를 해결할 수 있었다.

### 1. 네트워크 트래픽 분산 (임시 조치)

가장 먼저 시도한 것은 문제의 원인이 과도한 트래픽일 가능성을 줄여보는 것이었다. 이를 위해 로드 밸런서(FortiGate Virtual Server)의 분배 방식을 조정했다.

- <b>방법</b>: 기존의 균등 분배(Round Robin) 방식 대신, 문제가 발생한 노드의 가중치(weight)를 낮추고 안정적인 다른 노드의 가중치를 높여 트래픽을 최소화했다.
- <b>결과</b>: 이 방법은 일시적으로 증상을 완화할 수는 있었지만, 근본적인 해결책은 아니라고 판단했다.
...
> [!IMPORTANT]
> <b>e1000e hang 이슈의 가장 유력한 원인</b>은 드라이버의 전력 관리 기능과 관련된 것으로 알려져 있다. 대부분의 경우 이 기능을 비활성화하는 것만으로도 문제가 해결될 수 있다.
