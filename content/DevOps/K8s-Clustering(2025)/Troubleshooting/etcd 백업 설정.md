...
type:
  - note
---
예전에 쿠버네티스 클러스터를 구성했을 때 etcd 를 백업하지 않은채 노드 join 을 하다 etcd 클러스터 장애가 발생하고 결국 쿠버네티스 클러스터 전체를 재구성 했던 적이 있다. 운영서버가 아니었기 때문에 망정이지 정말 땀나는 경험이 아닐 수 없었다. 그러다보니 자연스레 백업은 생활이 된 것 같다.
