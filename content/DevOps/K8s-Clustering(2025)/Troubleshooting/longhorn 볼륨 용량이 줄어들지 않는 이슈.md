...
Longhorn이 볼륨 용량을 줄이지 않는 근본적인 이유는 다음과 같다.

1. **블록 레벨 동작**: Longhorn은 파일 시스템 수준이 아닌 <b>블록 수준</b>에서 작동한다
2. **정보 부족**: 사용자가 파일 시스템에서 콘텐츠를 제거했는지 여부를 블록 스토리지 계층에서는 알 수 없다
3. **Thin Provisioning**: 동적으로 할당된 블록들이 파일 삭제 후에도 자동으로 해제되지 않는다

### 해결 방법 1: fstrim 명령어 사용 (권장)

Longhorn <b>v1.4.0</b> 이상 버전부터는 볼륨 트리밍(Volume Trimming) 기능을 지원한다. 이 기능을 통해 사용하지 않는 블록을 수동으로 회수할 수 있다.
...
> [!IMPORTANT]
> fstrim 작업은 **I/O 집약적**이므로 서비스 중단 시간을 고려하여 수행해야 한다. 가능하면 트래픽이 적은 시간대에 실행하는 것을 권장한다.
