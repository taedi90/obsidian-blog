...
> [!SUMMARY]
> Weaviate는 클러스터 간 실시간 복제(CDC)가 없어 backup/restore로만 이관하면 쓰기 정지 시간이 데이터 크기에 비례한다. 그래서 backup/restore로 새 클러스터를 미리 채운 뒤, 그 이후 바뀐 오브젝트만 REST `after` 커서로 전량 순회하며 UUID 보존 upsert로 따라잡는 증분 동기화 스크립트를 stdlib만으로 짰다. 컷오버 때는 마지막 소량 델타만 반영하면 되니 중단창이 "전체 복원"에서 "잔여분 반영"으로 줄어든다.
...
<b>생성·수정은 그나마 방법이 있다.</b> 오브젝트마다 `lastUpdateTimeUnix`(마지막 수정 시각, unix ms)가 노출된다. 그러니 "시딩 시각 이후 수정된 오브젝트"를 골라낼 여지는 있다. 다만 이걸 서버 측 `where` 필터로 거는 건 이 저장소 버전(1.36)에서 불안정했다. 시간 필터가 기대대로 안 걸려서, 결국 <b>클라이언트에서 걸러내는</b> 쪽으로 방향을 틀었다. 소스를 전량 순회하면서 각 오브젝트의 `lastUpdateTimeUnix`를 코드에서 비교하는 방식이다. 우아하진 않지만 서버 필터의 신뢰성에 기대지 않아도 된다.
...
이건 소스·타깃을 양쪽 다 전량 순회하니 느리다. 그래서 평소 델타 반영에는 끄고, append-only가 아닌 이상 컷오버 정합 단계에서만 켰다. 삭제 빈도를 앱팀이 확답 못 하면 켜두는 쪽을 택했다. "지운 문서가 검색에 계속 나오는" 조용한 버그가 안 켜서 생기는 손해보다 성가시기 때문이다. 안전장치로 구 클러스터와 Retain PV는 검증 끝날 때까지 남겨뒀다.
...
API 키는 `--api-key` 인자나 `WEAVIATE_API_KEY` 환경변수로만 받고, <b>어떤 로그에도 값을 출력하지 않는다</b>. 소스·타깃 키가 다르면 `--source-key`/`--target-key`로 따로 준다. 의존성은 없다. `urllib` 등 표준 라이브러리만 써서, 폐쇄망에 파이썬 인터프리터만 있으면 그대로 돌아간다. pip로 뭘 반입할지 협의하는 수고를 덜려고 일부러 그렇게 짰다.
