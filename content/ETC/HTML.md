...
<b>HTML(HyperText Markup Language)</b>은 웹 페이지의 콘텐츠와 구조를 정의하기 위한 <b>마크업 언어</b>이다. 프로그래밍 언어처럼 복잡한 로직을 수행하는 것이 아니라, <b>태그(Tag)</b>라는 약속된 표기법을 사용하여 텍스트, 이미지, 비디오 등의 콘텐츠를 감싸고 그 의미와 구조를 부여하는 역할을 한다.

브라우저는 HTML 코드를 해석하여 사용자에게 시각적인 웹 페이지로 보여줍니다. 흥미로운 점은 HTML 이 문법적으로 다소 관대하여 약간의 오류가 있더라도 브라우저가 최대한 페이지를 렌더링하려고 노력한다는 것이다. 하지만 이는 잠재적인 문제를 숨길 수 있으므로 웹 표준에 맞는 정확한 마크업을 작성하는 습관이 매우 중요한다.

이 문서는 HTML 의 가장 기본적인 구조부터 웹 페이지의 레이아웃을 구성하는 방법, 검색 엔진 최적화(SEO)와 웹 접근성에 필수적인 <b>시맨틱 웹(Semantic Web)</b>의 개념까지 핵심 내용을 정리하는 것을 목표로 한다.

## 2. HTML 문서의 기본 구조

모든 HTML 문서는 아래와 같은 기본적인 구조를 가집니다. 각 태그는 웹 브라우저와 검색 엔진에게 문서의 유형, 언어, 문자 인코딩 방식 등 중요한 정보를 전달한다.
...
HTML 요소는 크게 <b>블록 레벨 요소</b>와 <b>인라인 레벨 요소</b> 두 가지로 나뉩니다. 이 둘의 가장 큰 차이는 화면에 표시될 때 공간을 차지하는 방식이다.
...
-   <b>인라인 레벨 요소(Inline-level elements)</b>
    ...
    -   문장이나 단어처럼 작은 부분에 스타일을 적용할 때 주로 사용된다.
...
<b>시맨틱 HTML(Semantic HTML)</b>은 태그 자체가 콘텐츠가 지니는 의미를 명확하게 설명해 주는 것을 말한다. 예를 들어 `<div>` 대신 `<header>`, `<nav>`, `<main>`, `<footer>` 와 같은 시맨틱 태그를 사용하면 브라우저와 개발자 모두 해당 영역의 역할을 쉽게 파악할 수 있다.
...
> [!NOTE]
> `<table>` 태그는 데이터를 표 형태로 표현하기 위한 시맨틱 태그이다. 과거에는 레이아웃을 잡기 위해 사용되기도 했지만 이는 태그의 본래 의미에 맞지 않는 사용법이므로 지양해야 한다.
...
Emmet 은 HTML 과 CSS 코드를 매우 빠르고 효율적으로 작성할 수 있도록 도와주는 플러그인이다. 대부분의 최신 코드 에디터에 내장되어 있으며 간단한 축약 문법을 통해 복잡한 HTML 구조를 한 번에 생성할 수 있다.
