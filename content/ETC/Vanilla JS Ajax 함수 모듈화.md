...
completed:
---
개인 프로젝트나 팀 프로젝트를 가릴 것 없이 Ajax 를 매우 많은 곳에 사용했는데, 그때마다 코드를 작성하려니 `contentType` 처럼 외우기 까다로운 부분도 있고 코드도 길어진다고 판단하여 모듈화하여 사용했습니다.

개인적으로 `post` 방식으로 `json` 데이터를 사용하는 경우가 많아서 기본값으로 두었고, 데이터 형식은 "`json / multipart / urlencoded`" 에 해당하는 경우만 고려했습니다.

코드는 짧지만 깔끔해 보이지 않아서 한참 동안 고민했지만 결국 만족할 만큼 다듬지 못한 것 같습니다.

# 시도해본 것

- 필요한 전달인자만 입력하면 되도록 구성
