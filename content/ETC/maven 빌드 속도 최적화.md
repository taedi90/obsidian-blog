...
## 2. 이슈
프로젝트를 로컬에서 테스트할 때마다 generated-sources 변경이나 협업자들의 추가 개발 사항에 맞추어 빌드를 자주 수행하게 되는데, 테스트를 건너뛰어도 매번 2분 이상의 빌드 시간이 발생하는 것이 여간 불편하게 느껴지지 않을 수 없었습니다.
...
코딩도 역시 장비빨인가요? 프로세서를 교체하면서 빌드 시간이 획기적으로 단축되었습니다. 물론 PC 자체가 바뀐 것이므로 프로세서 외에 메모리나 저장장치 쪽 개선도 있겠지만, 어쨌든 만족스러운 결과입니다. ~~애플 실리콘 맥 최고..!~~

### 적용 불가
- 컴파일러 버전 최신화: 프로젝트 라이브러리 중 lombok 이 컴파일러 버전과 호환되지 않아 테스트가 불가능했습니다.
