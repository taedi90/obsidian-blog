...
Cobra를 마음 편히 고른 데엔 이유가 하나 더 있었다. 내가 매일 쓰는 도구들이 이걸로 만들어졌다. `kubectl`, `helm`, `kubeadm`이 전부 spf13/cobra 기반이고, 그 바깥으로도 `docker`·`gh`·`hugo`가 같은 계열이다. (Kubernetes 진영이 워낙 많이 써서 Go CLI의 표준처럼 자리 잡았다.)
