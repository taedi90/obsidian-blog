---
title: 레거시 Python 배포 봇을 Go로 재작성해 CI/CD를 Slack 한 창구로 일원화하기
date: 2026-07-01
draft: false
featured: true
tags:
...
banner: 
cssclasses: 
description: 결과와 무관하게 성공을 찍고, 재시작 한 번에 감시가 통째로 사라지던 Python 배포 봇을 Go로 다시 써서 신뢰할 수 있게 만든 기록.
permalink: 
aliases: 
completed: true
type:
...
> [!SUMMARY]
> CI → GitOps PR → CD로 이어지는 배포 흐름을 Slack 한 창구로 묶어 쓰고 있었는데, 그 창구였던 Python 봇이 <b>빌드가 실패해도 초록 체크를 찍고</b>, 프로세스가 한 번 재시작되면 <b>진행 중이던 배포 감시가 통째로 사라지는</b> 물건이었다. 봇을 Go로 다시 썼다. Socket Mode로 Slack에 붙고 Jenkins REST를 폴링해 실제 결과를 추적하며, 승인 게이트 두 곳을 `wfapi/pendingInputActions`로 감지해 게이트별로 의미를 갈라 보고하고, 부팅 시 진행 중인 빌드를 Jenkins에 되물어 로컬 상태나 DB 없이 감시를 자동 재부착하고, 권한은 fail-closed 게이트로 잠그고 자격증명은 최소 스코프로 쪼갰다.

## 1. 왜 배포를 Slack 한 창구로 묶었나

먼저 이 봇이 왜 있는지부터. 우리 개발 클러스터는 폐쇄망 안에 있어서, 배포를 하려면 VPN을 물고 사내 Jenkins UI에 들어가 잡을 찾고 파라미터를 채워 눌러야 했다. QA가 "이 브랜치 좀 검증 환경에 올려주세요"라고 하면 그 절차를 아는 사람이 대신 눌러줘야 했고, 그게 대부분 나였다.

그래서 흐름 전체를 Slack 한 창구로 묶었다. 채널에서 슬래시 커맨드를 치면 파라미터 모달이 뜨고, 제출하면 Jenkins 빌드가 돌고, 중간에 필요한 승인(DB 스키마 마이그레이션 적용, 이미지 Push)은 <b>Slack 버튼</b>으로 받고, GitOps PR이 생기면 링크와 <b>Merge/Close 버튼</b>을 채널에 띄운다. Merge하면 ArgoCD가 롤아웃한다. VPN 없이도, Jenkins 잡 구조를 몰라도, 슬랙만 볼 줄 알면 배포를 굴릴 수 있다는 게 핵심이었다.

의도 자체는 나쁘지 않았다. 문제는 그 창구가 거짓말을 한다는 데 있었다.

## 2. 구 Python 봇의 세 가지 결함

기존 봇은 Python으로 짜여 있었고, 겉보기엔 잘 돌았다. 겉보기엔.
...
```python
# 구 Python 봇: 빌드가 끝났거나 승인 대기에 걸리면 결과를 갱신하는 부분
...
```python
# 구 Python 봇: 감시 로직을 데몬 스레드로 백그라운드에 던진다
...
```python
# 구 Python 봇 config: Jenkins 접속 계정 기본값이 admin
"JENKINS_USER": environ.get("JENKINS_USER", "admin"),
```

Jenkins `admin` 계정 하나로 빌드도 트리거하고 상태도 읽었다. 여기에 GitHub 토큰, ArgoCD 토큰까지 한 봇이 다 쥐고 있었는데, ArgoCD 쪽은 사실상 관리자 권한 토큰을 그대로 넣어 쓰는 구조였다. 봇 컨테이너 하나가 털리면 CI 서버 admin + GitOps 저장소 쓰기 + ArgoCD 관리자가 한꺼번에 넘어간다. blast radius(사고 반경)가 봇 한 대에 다 걸려 있었다. 접근성을 위해 Slack으로 문을 넓혔으면, 그 문 뒤에 있는 권한은 오히려 더 좁혀야 했는데 그 반대였다.

> [!NOTE]
> 이 자격증명 문제는 범위가 커서 따로 위협 모델로 정리했다. blast-radius 관점의 분석과 rotate·fail-closed·HMAC·스코프 서비스계정으로 닫은 과정은 [[배포 봇에 몰린 admin 자격증명을 위협 모델로 정리하기]]에 있다. 아래 6절은 그중 이 재작성에서 코드로 바로 반영한 부분만 다룬다.

## 3. Go 재작성: 실제 결과 추적

패치로 덧댈 수도 있었지만, 세 결함이 다 상태 관리에서 나온 거라 봇을 Go로 다시 쓰기로 했다. 언어를 바꾼 건 취향이 아니라, 단일 바이너리로 배포되고 goroutine·context로 다수의 감시를 취소 가능하게 다루기 쉬운 쪽이 이 문제에 맞았기 때문이다.

Slack 연결은 기존과 같이 <b>Socket Mode</b>를 유지했다. 폐쇄망 안에서 도는 봇이라 외부에서 들어오는 인바운드 포트를 열 수 없고, Socket Mode는 봇이 아웃바운드 WebSocket으로 Slack에 붙기 때문에 이 제약에 딱 맞는다. 이 결정만은 구 봇에서 그대로 가져왔다.
...
```go
// Go 봇: 빌드가 끝났을 때(!building && res != "")에만 실제 result로 판단한다.
...
- <b>앞 게이트(마이그레이션)에서 중단</b>: 스키마가 아직 안 적용된 상태다. 여기서 끊으면 배포가 아무 의미가 없다. <b>진짜 실패</b>다.
...
```go
// Go 봇: 부팅 시 Jenkins의 진행 중 빌드를 조회해 감시를 다시 붙인다
...
```go
// Go 봇: 허용 사용자 목록이 비면 전체 허용이 아니라 기동 자체를 거부한다
if len(c.AllowedUsers) == 0 {
    return nil, fmt.Errorf("ALLOWED_USERS is empty: fail-closed, refusing to allow all users")
}
```

구 봇의 `ALLOWED_USERS` 기본값은 `"*"`(전체 허용)였다. 설정을 깜빡하면 조용히 누구나 배포할 수 있게 열려버리는 것이다. fail-closed는 그 반대로, 설정을 깜빡하면 아무도 못 쓰게 잠긴다. 실수했을 때 안전한 쪽으로 넘어지게 만드는 게 게이트의 기본이라고 본다. 사용자 판별은 Slack user ID 직접 매칭에 실패하면 `users.info`로 프로필 이메일을 조회해 허용 목록과 대조하는데, 조회가 실패하거나 이메일이 비면 그대로 거부한다. 승인 버튼도 마찬가지다 — 버튼은 누구 눈에나 보이지만, 눌렀을 때 같은 판별을 거치므로 보이는 것과 누를 수 있는 것이 분리된다.
...
전부 다 만든 건 아니다. Jenkins가 PR 생성 시점에 봇으로 직접 웹훅을 쏘는 실시간 경로는 아직 폴링으로 대체 중이고, stage별 클러스터 라우팅도 더 다듬을 여지가 있다. PR을 머지한 뒤 ArgoCD가 정말 그 리비전에서 Healthy가 됐는지 확인하는 부분은 따로 [[GitOps 배포 봇의 ArgoCD 헬스 게이트 - stale-Healthy 오탐 제거|헬스 게이트]]로 정리했다. 성능이 몇 배 좋아졌다거나 하는 정량 지표를 들이밀 생각은 없다. 이번 작업은 속도가 아니라 <b>신뢰성</b>을 고친 거다. 실패를 실패라고 말하고, 죽어도 되살아나게 했다.

false-success와 상태 소실은 둘 다 봇이 자기 기억을 진실로 믿어서 생긴 결함이었다. 진실을 Jenkins에 맡기고 봇은 stateless하게 둔 게 이번 재작성에서 바꾼 핵심이다.
