...
그래서 전제를 하나 정하고 시작했다. 인터페이스(Slack에서 전부 처리한다)는 건드리지 않는다. 손볼 대상은 <b>봇이 admin 자격증명을 직접 쥐고 있다는 사실</b> 쪽이다. 사용자는 여전히 버튼만 누르고, 실제 권한 실행은 해당 권한만 가진 실행자에게 넘긴다. 이 방향이면 UX는 그대로 두고 위험만 줄일 수 있다. 관심사를 인터페이스와 자격증명으로 분리하니 "역할 과다"라는 막연한 불만이 실제로 고칠 수 있는 항목들로 나뉘었다.
