...
결론부터 말하면 이 가설은 대체로 틀렸다. 다만 왜 틀렸는지가 이 글의 핵심이므로, 순서대로 적는다.
...
건질 수 있는 것은 확실히 건진다. `complete`로 확인된 8개는 노드에서 곧바로 `ctr image push`로 재구축한 Harbor에 밀어 넣었다. 굳이 tar로 뽑아 옮길 필요 없이, 노드의 containerd가 가진 이미지를 레지스트리로 직접 올리면 된다.
