...
원래 CD 파이프라인의 스키마 마이그레이션은 단순했다. 외부 Jenkins 에이전트가 DB의 외부 NodePort로 직접 붙어서 마이그레이션 도구를 실행하고, 접속 정보는 파이프라인 설정 파일에 <b>평문 root 크레덴셜</b>로 박혀 있었다. 부끄럽지만 그것이 v1이었다. (지금 와서 보면 평문이 박혀 있던 것부터가 숙제였다.)
