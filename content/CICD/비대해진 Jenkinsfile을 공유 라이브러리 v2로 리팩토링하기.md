...
진단부터 정량으로 기록해뒀다. 5개 Jenkinsfile이 <b>약 4,100줄, 상당 부분이 파일 간 중복</b>이었다. 공유 라이브러리는 0개였고, 전부 Jenkins UI에 붙은 인라인 스크립트였다. 중복 자체보다 더 심각한 것은 그 중복이 이미 <b>갈라지기(drift)</b>를 시작했다는 사실이었다.
...
멀티브랜치 watch 같은 기능을 하나 더 얹기 전에 기반부터 다시 다져야 했다. 기능을 추가하기 전에 <b>파이프라인을 유지보수하고 검증하는 방법론을 먼저 정립</b>하는 것이 이 작업의 목표였다.
