...
- <b>실제 작업에 쓰는 메모리</b>(working set): 회수하면 프로세스가 죽거나 느려지는 부분이다.
- <b>page cache</b>: 파일 I/O를 위해 커널이 캐싱해둔 것이다. <b>메모리가 부족하면 커널이 알아서 회수</b>한다. "쓰는 중"이라기보다 "여유가 있어 캐싱해둔 것"에 가깝다.
