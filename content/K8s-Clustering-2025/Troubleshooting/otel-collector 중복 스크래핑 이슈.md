...
> [!SUMMARY]
> otel-collector를 [[Otel pipeline|agent(DaemonSet)+gateway]] 2단으로 나눈 뒤, 같은 대상을 두 계층이 <b>둘 다 스크래핑</b>해서 메트릭이 중복으로 들어오는 문제가 있었다. 카운트가 부풀고 집계·알람이 틀어진다. 해결은 "누가 무엇을 긁을지"를 명확히 나누는 것 — 노드-로컬 대상(kubelet·파드 등)은 <b>자기 노드 것만</b> agent가 긁도록 스코프를 걸고, 클러스터 단위 대상은 gateway 한 곳에서만 긁게 했다.
...
- <b>DaemonSet agent가 노드 스코프를 안 걸었을 때</b> — 노드가 N개면 같은 클러스터-와이드 대상을 N개 agent가 각각 긁는다. 그대로 N중 중복.
- <b>agent와 gateway가 같은 대상을 둘 다 긁을 때</b> — 계층은 나눴는데 스크래핑 책임을 안 나눠서 이중 수집.
