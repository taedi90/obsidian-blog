...
오퍼레이터는 편하다. `MariaDB` CR 하나만 정의하면 StatefulSet, Service, Galera 부트스트랩, primary failover까지 자동으로 구성된다. 그런데 <b>백업을 복원해야 하는 순간</b>에는 이 편리함이 정확히 반대로 작동한다.

문제는 두 겹이다.

첫째, Galera는 논리적으로 데이터를 하나로 묶는 대신 노드마다 상태를 기억한다. 각 노드의 datadir에는 `grastate.dat`이 있고, 여기 적힌 `uuid`·`seqno`·`safe_to_bootstrap`으로 "누가 최신인지, 누가 클러스터를 처음 띄울 자격이 있는지"를 판단한다. 백업 데이터를 아무 노드에나 넣고 재기동하면, 노드들이 서로 다른 상태를 보고 <b>누구를 기준으로 클러스터를 구성할지 합의하지 못한다.</b> `NON_PRIM`에서 멈추거나, 엉뚱한 노드가 donor가 되어 방금 복원한 데이터를 SST로 덮어쓴다.

둘째, 그 위에 오퍼레이터가 있다. 오퍼레이터는 클러스터가 not-Ready면 "고쳐야 한다"고 판단하고 reconcile 루프를 돈다. 내가 손으로 노드를 재우고 datadir을 만지는 동안, 오퍼레이터는 그것을 <b>고장으로 인식하고 원래 상태로 되돌리려 한다.</b> 웹훅은 내가 CR을 수정하는 걸 막고, cert-controller는 또 자기 역할대로 인증서를 채운다. 사람과 컨트롤러가 같은 리소스를 두고 서로 다른 방향으로 조작하는 상황이 되어 작업이 매우 번거로워진다.
...
2. 오퍼레이터·웹훅·cert-controller를 `replicas 0`으로 중지한다.
...
`mariabackup`은 InnoDB 데이터 파일을 잠그지 않고 통째로 복사하는 물리 백업 도구다(`mariadb-backup`과 동일). 파일을 복사하는 동안에도 DB는 계속 쓰기를 받으므로, 복사된 파일들은 서로 <b>미묘하게 다른 시점의 스냅샷</b>이다. 그래서 복원 전에 반드시 `--prepare`로 redo/undo 로그를 적용해 한 시점으로 일관성을 맞춰야 한다. 이 단계를 건너뛴 데이터는 사실상 깨진 데이터와 같다.
...
> [!IMPORTANT]
> 원본 운영 스크립트에는 `--password=<평문>`이 그대로 포함돼 있었다. 이 방식은 프로세스 목록(`ps`)과 셸 히스토리에 값이 그대로 남는다. 실제로는 `~/.my.cnf`의 `[mariabackup]` 섹션이나 `MYSQL_PWD`, 혹은 K8s라면 `secretKeyRef`로 주입하는 게 맞다. 이 글의 명령에서는 비밀번호를 전부 제외했다(`<REDACTED>`).

복원은 세 단계다. 스트림을 풀고(`mbstream -x`), `--prepare`로 일관성을 맞춘 뒤, `--copy-back`으로 실제 datadir에 복사한다.
...
하지만 datadir을 직접 만지고 StatefulSet/PVC까지 손대는 이번 같은 작업에서는, 컨트롤러 자체를 중지하는 것이 확실하다. 오퍼레이터 본체와 함께 <b>웹훅(admission)·cert-controller</b>까지 함께 중지해야 한다. 웹훅이 살아있으면 CR 수정이 반려되고, cert-controller가 살아있으면 인증서 시크릿을 다시 채우려 들기 때문이다.
...
> [!INFO]
> 작업이 끝나면 이 셋을 다시 원래 레플리카 수로 복구해야 오퍼레이터가 정상 관리 상태로 돌아온다. 중지한 뒤 복구를 잊으면, 나중에 진짜 장애가 났을 때 자동복구가 안 도는 사고로 이어진다. (이후의 작업을 위해 남겨두는 메모다.)

### 3. 0번 노드에만 복원하고 grastate.dat 재작성

방법 자체는 단순하다. <b>0번 노드를 유일한 기준 상태로 만들고, 나머지 노드는 비워둔다.</b>

먼저 0번 Pod를 지우고 StatefulSet을 0으로 내려, mysqld가 뜨지 않는 상태에서 PVC를 직접 다룬다.
...
여기서 두 값이 핵심이다. `seqno: -1`은 "정상 종료 좌표를 모른다 = 복구 상황이다"라는 신호이고, `safe_to_bootstrap: 1`은 "이 노드로 클러스터를 처음 구성해도 된다"는 명시적 허가다. 이 둘이 있어야 Galera가 이 노드를 새 클러스터의 시작점으로 인정한다. 반대로 나머지 1·2번 노드의 PVC 데이터는 <b>전부 비운다.</b> 그래야 그 노드들이 부팅할 때 자기 데이터가 없어서 0번을 donor로 SST를 받고, 결과적으로 0번의 데이터로 클러스터가 통일된다.

> [!NOTE]
> 예전에 `mariadb-operator` + `MariaDB 11.0.3` 조합에서, `grastate.dat`이 0바이트가 되면 오퍼레이터의 agent가 `safe_to_bootstrap`을 갱신하지 못해 bootstrap API가 500을 반환하는 문제를 겪은 적이 있다. `error unmarshalling galera state: invalid galera state file` 같은 메시지다. 파일이 "존재하면서 최소한의 유효 형식을 갖추는 것"이 생각보다 중요하다. (당시엔 커스텀 이미지 wrapper로 이 파일을 감시·재생성하게 우회했다.)

필요하면 0번 노드의 설정에서 `wsrep_cluster_address`를 잠깐 `gcomm://`로 두어, 다른 노드를 찾지 않고 자기 혼자 새 클러스터를 시작한다는 것을 명시할 수도 있다. `gcomm://` 뒤가 비어 있다는 건 "join할 기존 노드가 없다 = 내가 시작점이다"라는 뜻이다.
...
### 4. 지금까지의 과정을 오퍼레이터에게 위임하기

3번까지가 "직접 수행하는" 표준 절차다. 그런데 `mariadb-operator`는 이 과정을 상당 부분 선언적으로 대신 해준다. 백업은 `PhysicalBackup` 리소스로 만든다. 스토리지는 VolumeSnapshot 또는 PVC 중 하나를 고른다.
...
이걸 적용하면 오퍼레이터가 노드 수만큼 `...-physicalbackup-init` Job을 띄운다. 각 Job은 두 컨테이너로 3번 절차의 수작업을 그대로 재현한다.

- init 컨테이너(오퍼레이터 이미지): `backup restore --path /backup --target-time 2025-09-16T00:00:00Z --backup-content-type Physical ...` 로 지정 시점에 맞는 백업 파일을 선택해준다.
...
즉 오퍼레이터가 내부에서 하는 일도 `prepare` → `copy-back`이다. 추상화 내부에서 동작하는 것은 mariabackup 그 자체라, 손으로 하든 위임하든 막히는 지점은 같다.

> [!INFO]
> `bootstrapFrom`으로 복원한 뒤에는, 초기 부트스트랩을 마친 `MariaDB` 리소스(및 필요 시 남은 init 관련 StatefulSet/Job)를 정리하는 후속 절차가 붙는다. 자동화가 편리하기는 해도, 스테이징 PVC 용량 부족이나 이미지 pull 실패로 Job이 멈추는 경우가 있어서 나는 여전히 3번의 수동 절차를 숙지하고 둔다.

## 5. 확인

복원이 끝나면 Galera가 하나의 클러스터로 합의했는지부터 확인한다. 살아있는 노드에서 wsrep 상태를 조회한다.
...
`wsrep_cluster_size=3`, `wsrep_cluster_status=Primary`, `wsrep_local_state_comment=Synced`, 그리고 `Ready=True`·`GaleraReady=True`면 끝이다. 마지막으로 복원한 시점의 데이터가 실제로 들어있는지 애플리케이션 테이블 몇 개를 눈으로 확인해야 안심할 수 있다. (백업이 최신이 아닐 수 있다는 걸 늘 의심하는 편이 낫다.)
