...
banner: 
cssclasses: 
description: "스테이징을 사내 표준 차트로 전환할 때 기존 PV 재부착이 기본 계획이었는데 ClickHouse 만 fresh 로 결정돼 트레이스 이력을 버렸다. 스토리지 기본 reclaim 이 Retain 이라 원본이 살아 있어서 임시 ClickHouse 를 띄워 되살렸고, 그런데도 화면에 안 보여서 정렬 키 컬럼인 project_id 를 재기록해 프로젝트를 합쳤다. 두 번의 복구 기록."
permalink: 
aliases: 
completed: true
type:
...
> [!SUMMARY]
> 데이터 이관 계획은 기존 PV 재부착이었는데 ClickHouse 만 "트레이스 이력만 초기화"로 판단해 fresh 로 갔다. 그 판단이 나중에 뒤집혀서 되살려야 했고, 스토리지 기본 reclaim 이 Retain 이라 원본이 남아 있어서 그 위에 임시 ClickHouse 를 띄워 `remote()` INSERT 로 옮겼다. 그런데 행을 다 넣고도 화면에는 안 보였다 — 차트가 프로젝트를 하나 새로 만들어 놨기 때문이고, 정렬 키 컬럼인 `project_id` 를 재기록해서 합쳤다.
...
스테이징을 사내 표준 차트로 전환하면서 백킹 서비스를 전부 신 형상으로 갈아탔다. <b>데이터 이관의 기본 방침은 기존 PV 재부착</b>이었다. 대용량은 재부착으로 즉시 처리하고 논리 이관(mariadb 덤프 30~60분, postgres 수분)만 시간을 쓴다는 계산으로 중단 창을 반일 하단으로 잡았다. 산정표에도 `ClickHouse 47G/노드(≈141G, 재부착)` 로 적혀 있다.
...
다른 컴포넌트는 같은 NFS export 안에서 구 데이터 dir 을 신 subdir 이름으로 `mv` 하면 즉시 재부착된다. minio·gitea·mongodb·registry 가 다 그 방식으로 살았다. ClickHouse 는 두 가지가 겹쳐 그게 안 됐다 — 구 형상이 subdir 없이 export 루트를 통째 마운트하고 있었고, CHI 이름이 바뀌면서 operator 가 만드는 PVC 이름과 경로가 달라졌다.

그래서 <b>버리기로 했다.</b> "트레이스 히스토리만 초기화"라는 문장이 그 판단이다. 관측 이력은 앱 동작에 필요한 상태가 아니고, 승계는 별도 정밀 작업이라고 봤다. 그 결정의 부수 효과로 Retain 선패치 대상에서도 빠졌다 — "fresh 결정이라 Delete 유지".
...
버리기로 한 데이터가 정말 버려도 되는 데이터였는지를 다시 묻는 질문이다. 그리고 답은 <b>아니오</b>였다.

## 3. 복구

### 1. 백업이 없을 때 유실을 무엇으로 판정하나

mariadb 는 전환 시점 덤프가 NAS 에 있어서 대조가 쉬웠다. ClickHouse 는 <b>백업 PVC 자체가 없었다.</b> 비교 기준이 없다.
...
TTL 을 먼저 확인했다. `traces`/`observations` 는 TTL 이 없다. 즉 만료로 지워진 게 아니라 애초에 안 넘어온 것이다.
...
여기가 좀 아이러니한데, 이 볼륨은 <b>Retain 선패치 대상에서 일부러 빠져 있었다.</b> fresh 로 가기로 했으니 롤백 보험이 필요 없다고 봤고, 체크리스트에도 "Delete 유지"로 적어뒀다. 살아남은 건 그 StorageClass 의 기본 reclaim 이 이미 Retain 이었던 덕이다. 버리기로 한 데이터가 스토리지 기본값 때문에 남아 있었고, 그게 나중에 유일한 복구 소스가 됐다. 이게 없었으면 이 글은 여기서 끝났다.

### 3. 복구 전에 확인한 정합성 의존성

행만 밀어넣으면 되는 게 아니다. langfuse v3 는 이벤트 페이로드(input/output)를 오브젝트 스토리지로 offload 하고 ClickHouse 에는 참조만 둔다. 그리고 trace 는 Postgres 의 프로젝트에 귀속된다. <b>둘 중 하나만 없어도 복구한 trace 가 고아가 된다.</b>
...
의존성이 다 살아 있었다. ClickHouse 하나만 빠진 상황이라 오히려 복구 조건은 깔끔했다.
...
파일을 옮기는 게 아니라 <b>구 데이터 위에 ClickHouse 를 하나 띄워서 SQL 로 읽어 내보내는</b> 방식이다. 파트를 파일시스템으로 신 테이블에 밀어넣고 keeper 에 수동 등록하는 길도 있지만, Replicated 테이블의 zk 경로는 테이블 UUID 기반이라 신 테이블 입장에서는 완전히 다른 테이블이다. 그 길로 가면 replica 이름·매크로까지 다 맞춰야 한다. `INSERT` 로 넣으면 신 테이블이 자기 UUID 아래에서 3노드 복제를 알아서 관리하므로 그 문제가 아예 사라진다.

그리고 실행 전에 이 말이 나왔다.

> "복구했을 때, 문제가 발생하지는 않을지 확인해줘"

이 한마디가 방식을 바꿨다. 구 데이터는 <b>이 세상에 하나 남은 백업</b>이고, ClickHouse 26.3 이 그 파트를 RW 로 열면 자동 업그레이드로 원본을 변형시킬 수 있다. 백업을 읽으려다 백업을 깨는 건 최악이다. 그래서 안전장치를 세 겹으로 깔았다.

- 구 노드-0 을 <b>42G tar.gz 로 압축 보존</b> (별도 아카이브)
- 임시 인스턴스는 <b>노드-1 만</b> 마운트. 3 replica 라 노드-2 는 손대지 않은 pristine 복제본으로 남는다
- keeper 를 unreachable 로 둬서 Replicated 테이블이 <b>readonly 로 attach</b> 되게 함

임시 인스턴스는 라이브와 같은 이미지 태그로 띄웠다(구 파트 하위호환 읽기).

첫 attach 는 실패했는데, 원인이 keeper 가 아니라 <b>매크로 `{shard}`/`{replica}` 미정의</b>였다. Replicated 엔진의 zk 경로에 매크로가 들어가 있으니 정의가 없으면 테이블을 열 수가 없다. 매크로만 채우고 keeper 는 계속 못 닿게 두면 readonly 로 읽힌다.

스키마 차이도 걱정거리였다. 구 langfuse 는 테이블을 `default` DB 에 뒀고 신은 `langfuse` DB 다. 버전이 다르면 컬럼 매핑을 손으로 짜야 한다. 그런데 `schema_migrations` 를 세어보니 양쪽 다 68 이었다. <b>스키마가 동일해서 `SELECT *` 로 그대로 옮길 수 있었다.</b> 여기서 작업량이 절반으로 줄었다.
...
이동은 라이브에서 임시 인스턴스를 `remote()` 로 당겨오려 했는데, 임시 쪽이 원격 접속의 빈 비밀번호를 거부했다(로컬은 되는데 원격 인증은 별도다). 비밀번호를 아는 쪽이 라이브니까 <b>방향을 뒤집어 임시 → 라이브 push</b> 로 우회했다.

### 5. 복구 중 만난 함정 넷

<b>50GB 를 한 번에 흘리다 replica 가 readonly 로 떨어졌다</b>

`observations` 50.4GiB 를 단일 INSERT 로 스트리밍하다 대상 테이블이 `Code 242: Table is in readonly mode` 로 실패했다. 120,195/881,822 에서 중단. ReplicatedMergeTree replica 가 keeper 세션을 순간 잃으면 readonly 가 되는데, 대용량 인서트 부하 중의 일시 현상이었고 곧 회복됐다. 문제는 <b>한 방에 하다 중간에 맞으면 통째로 날아간다</b>는 것이다. 그래서 월 파티션 단위 배치 + 재시도로 재개했다. ReplacingMergeTree 라 재적재가 멱등이어서 부분 적재된 12만 행도 그냥 덮으면 됐다.

<b>kubectl exec 스트림이 반복해서 끊겼다 — 그런데 서버측 쿼리는 안 죽었다</b>

긴 INSERT 를 `kubectl exec` 에 매달아 두면 API 서버 연결이 `connection reset by peer` 로 끊긴다. 이건 예상했다. 예상 못 한 건 <b>클라이언트가 죽어도 서버측 INSERT 는 계속 살아 있다</b>는 것이다. 재시도가 겹치면서 같은 INSERT 가 동시에 여러 개 돌고 있었다. 앞의 일시 readonly 도 이 경합 부하 탓으로 보인다.

잔류 쿼리를 전부 KILL 하고, 파드 내부에 스크립트를 심어 detach 로 돌렸다.
...
완료 판정을 `system.processes` 폴링으로 했는데, 조건이 `query LIKE '%remote%observations%'` 였다. 이 판정 쿼리 자신이 그 패턴에 걸린다. 그래서 영원히 "아직 실행 중"이 되어 다음 단계로 못 넘어갔다. 데이터에는 무해했지만 한 시간을 헛돌았다. (자기매칭, 고전이다.)
...
멱등한 엔진에 익숙해지면 이 구분을 놓치기 쉽다. 같은 스크립트로 다 밀면 안 되는 이유다.
...
4천 행이 없다. 순간 서늘했는데, 세는 방식이 달랐다. 구 원본은 `raw`(병합 안 된 중복 버전 포함)이고 신 라이브는 `FINAL`(dedup)이다. langfuse 는 같은 레코드를 `event_ts` 버전으로 여러 번 쓰므로 ReplacingMergeTree 로 접히는 게 정상이다. 사과 대 오렌지를 비교한 것이다.
