...
오랫동안 방치해 두었던 시놀로지 나스의 포트 번호가 기억나지 않아 전체 포트를 스캔해야 하는 상황이 발생했다. 그런데 `tcping [example.com] [port]` 명령어로 조회하려면 65535번을 반복해서 확인해야 했기 때문에, 다른 명령어를 찾아보게 되었다.
...
- 전체 UDP 포트 스캔  
    UDP 포트 스캔을 수행하려면 sudo 권한이 필요하다.  
